...
└─ examples/
   ├─ prompt_examples.md
   ├─ sample_review_request.md
   ├─ sample_review_payload.json
   ├─ sample_review_generated.hwpx
...
```text
$hwp 스킬을 사용해서 examples/blank_review_template.hwpx 느낌으로 신규 산업단지 진입도로 폭 검토서를 작성해줘.
```

```text
$hwp 스킬을 사용해서 examples/sample_review_request.md 내용을 기반으로 .hwpx 결과물을 생성해줘.
```
...
- `examples/sample_review_request.md`
  - 실제 사용 예시용 Markdown 입력 원본입니다.
- `examples/sample_review_payload.json`
  - 위 Markdown을 payload로 변환한 예시입니다.
- `examples/sample_review_generated.hwpx`
...
- 가장 빠른 방법은 `examples/sample_review_generated.hwpx`를 직접 열어보는 것입니다.
- 어떤 입력으로 생성됐는지 보려면 `examples/sample_review_request.md`를 먼저 보면 됩니다.
- Codex에서 재현하고 싶다면 아래처럼 말하면 됩니다.

```text
$hwp 스킬을 사용해서 examples/sample_review_request.md를 읽고 같은 형식의 hwpx를 다시 만들어줘.
```
